...
- **Per-recording sync report in the UI** — inter-camera sync error vs the frame period plus a clap-detection
  diagnostic, built and viewed entirely in the browser.
- **Post-hoc alignment** — audio-clap cross-correlation (primary) + per-frame timestamps (cross-check),
  resampled to a common constant frame rate with ffmpeg.
...
```bash
git clone <your-repo-url> "camera time sync"
cd "camera time sync"
...
1. On each iPhone (same Wi-Fi), open Safari and **scan the QR** in the control UI → **Allow camera & mic**.
   The phone appears in the **Cameras** table with a live clock offset.
2. Set capture config → **Push config to all**. Fill in the session title / subject / trial / notes.
3. **Start (synchronized)** → at the **CLAP** prompt, clap once (sharp, single impulse) → **Stop**.
4. Clips upload automatically to `sessions/<session_id>/<camera>/`.

### View the sync report

In the **Recordings & sync reports** panel, click **View report**. You get the inter-camera sync error vs the
frame period and the clap-detection diagnostic — no terminal needed.
...
1. **Audio clap cross-correlation (primary)** — the single physical clap recorded by every phone's mic gives
   a sharp, sub-frame relative offset.
2. **Per-frame timestamps (independent cross-check / fallback)** — first server-clock stamp per camera from
   `frames.json`.
3. **Resample** — ffmpeg renders each clip to a common constant frame rate with the claps coincident.
...
```bash
bash tests/run_all.sh          # host smoke (18/18) + offline sync validation (2/2)
